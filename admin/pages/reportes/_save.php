...
if (isset($action)== "upload"){
	...
	if (file_exists ("cop_".$archivo)){ 
		...
		for ($i=2;$i<=$filas;$i++){
								$_DATOS_EXCEL[$i]['id_user'] = $objPHPExcel->getActiveSheet()->getCell('A'.$i)->getCalculatedValue();
								$_DATOS_EXCEL[$i]['nombre'] = $objPHPExcel->getActiveSheet()->getCell('B'.$i)->getCalculatedValue();
								$_DATOS_EXCEL[$i]['acciones']= $objPHPExcel->getActiveSheet()->getCell('C'.$i)->getCalculatedValue();
								$_DATOS_EXCEL[$i]['valor_cuotaparte']= $objPHPExcel->getActiveSheet()->getCell('D'.$i)->getCalculatedValue();
								$_DATOS_EXCEL[$i]['fecha'] = $objPHPExcel->getActiveSheet()->getCell('E'.$i)->getCalculatedValue();
								$_DATOS_EXCEL[$i]['total_invertido'] = $objPHPExcel->getActiveSheet()->getCell('F'.$i)->getCalculatedValue();
								$_DATOS_EXCEL[$i]['activo'] = 1;
							}		
							$errores=0;

							$sqlFlush = "DELETE FROM tab_reportes";
							$resultFlush = mysqli_query($link, $sqlFlush);


		foreach($_DATOS_EXCEL as $campo => $valor){
								$sql = "INSERT INTO tab_reportes (id_user,nombre,acciones,valor_cuotaparte,fecha,total_invertido,activo)  VALUES ('";
								foreach ($valor as $campo2 => $valor2){
									$campo2 == "activo" ? $sql.= $valor2."');" : $sql.= $valor2."','";
								}

								if( $campo == 'id_user' && $valor == '0') {
									//registro espaciador
								}else {
